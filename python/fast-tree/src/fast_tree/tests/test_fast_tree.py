...
def test_fast_tree(thread_count):
    ...
    t = FastTree(key, key_for_sorting, key_for_search)
    items = t.items()

    def add(item):
        items[item["id"]] = item

    add({"id": "2", "name": "item 1", "comment": "12", "extra": "20"})
    add({"id": "1", "name": "item 1", "comment": "11", "extra": "10"})
    ...
    def assert_block(block_size, block_start, query, expected_ids, expected_total):
        b = t.get_block(block_size, block_start, query, thread_count, 2)
        rows = b["rows"]
